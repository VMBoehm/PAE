...
def conv2d(input_, output_dim, k_h, k_w, d_h, d_w, stddev=0.02, name="conv2d",
  ...
  with tf.compat.v1.variable_scope(name):
    ...
    biases = tf.compat.v1.get_variable(
        "biases", [output_dim],use_resource=False, initializer=tf.compat.v1.constant_initializer(0.0))
...
def linear(input_, output_size, scope=None, stddev=0.02, bias_start=0.0, use_sn=False):
    ...
    with tf.compat.v1.variable_scope(scope or "Linear"):
        matrix = tf.compat.v1.get_variable("Matrix", [shape[1], output_size], tf.float32, tf.compat.v1.random_normal_initializer(stddev=stddev),use_resource=False)
        bias = tf.compat.v1.get_variable("bias", [output_size], use_resource=False, initializer=tf.compat.v1.constant_initializer(bias_start))
        if use_sn:
...
def deconv2d(input_, output_shape, k_h, k_w, d_h, d_w, stddev=0.02, name="deconv2d"):
    with tf.compat.v1.variable_scope(name):
        w = tf.compat.v1.get_variable("w", [k_h, k_w, output_shape[-1], input_.get_shape()[-1]], use_resource=False, initializer=tf.compat.v1.random_normal_initializer(stddev=stddev))
        deconv = tf.nn.conv2d_transpose(input_, w, output_shape=output_shape, strides=[1, d_h, d_w, 1])
        biases = tf.compat.v1.get_variable("biases", [output_shape[-1]],use_resource=False, initializer=tf.compat.v1.constant_initializer(0.0))
