...
def mnist_resnet_decoder(params,is_training, reuse=tf.compat.v1.AUTO_REUSE):
    ...
    """
    def decoder(z):
        with tf.compat.v1.variable_scope('model/decoder', ['z'], reuse=reuse):
            net = tf.compat.v1.layers.dense(z, params['dense_size'], activation=None)
            ...
            net = tf.compat.v1.layers.dense(net, params['output_size'], activation=None)
        return net
...
def conv_encoder(params, is_training=True):
    ...
    def encoder(x):
        with tf.compat.v1.variable_scope('model/encoder',['x'], reuse=tf.compat.v1.AUTO_REUSE):
            ...
            net = tf.compat.v1.layers.conv2d(net, n_filt*2, 3, 2, activation=None, padding='SAME', use_bias=bias) #32x32 -> 16x16 / 14x14 -> 7*7
            net = tf.compat.v1.layers.batch_normalization(net, training=is_training)
            net = activation(net)
...
def conv_decoder(params, is_training=True):
    ...
    def decoder(z):
        with tf.compat.v1.variable_scope('model/decoder',['z'], reuse=tf.compat.v1.AUTO_REUSE):
            ...
            net = tf.compat.v1.layers.conv2d_transpose(net,n_filt*2, 3, strides=2, padding='SAME', use_bias=bias) # output_size 16x16/14x14 
            net = tf.compat.v1.layers.batch_normalization(net, training=is_training)
            net = activation(net)
...
def fully_connected_decoder(params,is_training):
    ...
    def decoder(z):
        with tf.compat.v1.variable_scope('model/decoder', reuse=tf.compat.v1.AUTO_REUSE):
            ...
            net = tf.compat.v1.layers.dense(net, params['output_size'] , name='dense_4', activation='sigmoid')
        return net-0.5
    return decoder
